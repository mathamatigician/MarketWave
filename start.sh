...
if check_port $EMULATOR_PORT; then
  ...
  setsid nohup npx -y firebase-tools@latest emulators:start --only firestore --project marketwave-demo > firestore-emulator.log 2>&1 < /dev/null &
  EMULATOR_PID=$!
...
if check_port $BACKEND_PORT; then
  ...
  setsid nohup $PYTHON_CMD -m uvicorn backend.main:app --host 0.0.0.0 --port $BACKEND_PORT --reload --reload-dir backend > backend.log 2>&1 < /dev/null &
  BACKEND_PID=$!
...
if check_port $FRONTEND_PORT; then
  ...
  setsid nohup npm --prefix frontend run dev -- --host 0.0.0.0 --port $FRONTEND_PORT > frontend.log 2>&1 < /dev/null &
  FRONTEND_PID=$!
