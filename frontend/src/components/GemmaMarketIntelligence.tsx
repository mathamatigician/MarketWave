...
import { RefreshCw, Sparkles, AlertCircle, Bot, Radio, CheckCircle2 } from 'lucide-react';
...
export const GemmaMarketIntelligence: React.FC<GemmaMarketIntelligenceProps> = ({
  ...
  return (
    <div className="rounded-xl border dark:border-indigo-500/20 border-indigo-200/60 dark:bg-[#111116] bg-slate-50/60 p-4 sm:p-5 shadow-sm space-y-4 transition-all">
      {/* Panel Header & Live Controls */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b dark:border-white/5 border-slate-200 pb-3">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-indigo-500 dark:text-indigo-400 animate-pulse shrink-0" />
            <h3 className="text-xs font-mono font-bold uppercase tracking-[0.2em] dark:text-white text-slate-900">
              ⚡ Gemma AI Market Intelligence
            </h3>
            <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full dark:bg-indigo-950/60 bg-indigo-100 text-indigo-600 dark:text-indigo-300 border border-indigo-500/20">
              {modelName || 'Google Gemma 3 (12B)'}
            </span>
          </div>
          <p className="text-[11px] font-mono text-slate-500 dark:text-white/40">
            Real-time AI synthesis of your active watchlist catalysts
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          {briefingTimestamp && !isUpdating && (
            <span className="text-[10px] font-mono text-slate-500 dark:text-white/40 bg-white dark:bg-white/5 px-2 py-1 rounded-md border dark:border-white/5 border-slate-200">
      ...
      {overallScore !== null && (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-500/5 border border-indigo-500/10 text-[11px] font-mono">
          ...
          <span className="text-slate-600 dark:text-white/70 truncate">
            Watchlist sentiment is {trendLabel.toLowerCase()} ({overallScore >= 0 ? `+${overallScore.toFixed(2)}` : overallScore.toFixed(2)}). Neural triage mapped {hasBriefing ? briefing.length : 0} catalyst channels.
          </span>
        </div>
      )}
      ...
      {hasBriefing ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {briefing.map((item, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="rounded-lg border dark:border-white/10 border-slate-200 dark:bg-white/2 bg-white p-3.5 space-y-2 hover:dark:border-indigo-500/30 hover:border-indigo-300 transition-all shadow-xs"
              >
                <div className="flex items-center justify-between gap-2 border-b dark:border-white/5 border-slate-100 pb-2">
                  <div className="flex items-center gap-1.5">
                    <Bot className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                    <span className="font-mono font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-white">
                      {item.ticker}
                    </span>
                  </div>
                ...
                <p className="text-[11px] leading-relaxed text-slate-600 dark:text-white/80 font-mono">
                  {item.bullet}
                </p>

                <div className="flex items-center justify-between pt-1 text-[9px] font-mono text-slate-400 dark:text-white/30">
                  <span className="flex items-center gap-1">
                    <CheckCircle2 className="w-2.5 h-2.5 text-emerald-500 dark:text-[#00FF94]" />
                    Real-time Ingested
                  </span>
                  <span>Impact Synthesized</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="p-8 text-center border border-dashed dark:border-white/10 border-slate-200 rounded-xl space-y-2">
