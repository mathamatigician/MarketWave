...
The pipeline loads [users.json](file:///Users/mathamatigician/datascience/Competitions/Hackathons/Gemini_Hackathon/MarketWave/users.json) and aggregates all tickers from all registered users' watchlists into a unique set (e.g., `['TSLA', 'AAPL', 'GOOGL', 'MSFT', 'NVDA', 'AMZN']`). If no user watchlists are defined, the pipeline falls back to this set of default stock symbols.
